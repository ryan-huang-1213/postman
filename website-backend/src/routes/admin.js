const { Log, AdminViewAllProfile, AdminViewProfile, FindAccountById, ConfirmPaymentStatus, UpdateAccountRoles, GetLogs, ChangeApplicationStatus, GetCampStatus, UpdateCampStatus, FindDataByName } = require("../Modules/Database");
const { ValidateRoles } = require("./../Modules/Validate");
const { AuthenticateAccessToken } = require("../Middleware/AuthenticateToken");

const express = require("express");
const { GenerateAccessToken } = require("../Modules/Tokens");
const router = express.Router();

async function CheckPermissions({ AccountId, CurrentRole, RequiredLevel, LogMessage }) {
    if (CurrentRole && ValidateRoles(CurrentRole, RequiredLevel)) {
        return [true, null];
    }
    ...
    if (LogMessage) {
        await Log(AccountId, Account.Role, LogMessage);
    }
    return [false];
}
